...
public class LookAtTarget : MonoBehaviour {
    ...
    [Tooltip("Speed at which the camera zooms in or out when pressing A or B")]
    ...
    void Update()
    {
       ...
       // if a currentTarget is set, then look at it
        ...
        if (currentTarget != null)
        {
            ...
            {
                Debug.Log("Touche B pressée");
                scroll = zoomSpeed * Time.deltaTime;
            }

            Vector3 direction = Camera.main.transform.forward * scroll;
            Vector3 newPos = Camera.main.transform.position + direction;

            newPos.x = Mathf.Clamp(newPos.x, -14f, 14f);
            newPos.y = Mathf.Clamp(newPos.y, -14f, 14f);
            newPos.z = Mathf.Clamp(newPos.z, -14f, 14f);

            float threshold = 0.1f;
            float minDistance = 0.75f;

            Vector3 delta = Camera.main.transform.position - currentTarget.transform.position;
            Vector3 targetPos = currentTarget.transform.position;

            if (!(Mathf.Abs(delta.x) < minDistance && Mathf.Abs(delta.x) > threshold) && !(Mathf.Abs(delta.y) < minDistance && Mathf.Abs(delta.y) > threshold) && !(Mathf.Abs(delta.z) < minDistance && Mathf.Abs(delta.z) > threshold) || Input.GetKey(KeyCode.V))
            {
